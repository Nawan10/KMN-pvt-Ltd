...
<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
    ...
    $(document).ready(function() {
        // Event listener for item selection dropdown change
        $('.item-select').change(function() {
            var itemId = $(this).data('item-id'); // Get the selected item ID
            var $priceInput = $(this).closest('tr').find('.price'); // Find the price input field in the same row
            
            // Send AJAX request to fetch item cost
            $.ajax({
                url: 'fetch_item_cost.php',
                method: 'POST',
                data: {itemId: itemId},
                success: function(response) {
                    // Update the price input field with the fetched item cost
